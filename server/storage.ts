...
export class DatabaseStorage implements IStorage {
  ...
  async createProjectTemplate(template: InsertProjectTemplate): Promise<ProjectTemplate> {
    ...
    const formattedTemplate = {
      ...template,
      techStack: Array.isArray(template.techStack) ? template.techStack : (template.techStack ? [template.techStack] : []),
      features: template.features ? (Array.isArray(template.features) ? template.features : [template.features]) : undefined,
      aiProviders: template.aiProviders ? (Array.isArray(template.aiProviders) ? template.aiProviders : [template.aiProviders]) : undefined,
    };
    
    const [newTemplate] = await db.insert(projectTemplates).values(formattedTemplate as any).returning();
    return newTemplate;
  }
